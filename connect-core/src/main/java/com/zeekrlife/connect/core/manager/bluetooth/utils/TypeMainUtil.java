...
public class TypeMainUtil {

    private static final String TAG = "TypeMainUtil";

    private static int getMainDeviceType(BluetoothDevice device) {
        int type = -1;
        BluetoothClass bluetoothClass = device.getBluetoothClass();
        if (bluetoothClass == null) {
            LogUtils.e(TAG, "getMainDeviceType device.name=" + device.getName() + ",bluetoothClass is null!!!");
            return type;
        }
        type = bluetoothClass.getMajorDeviceClass();
        LogUtils.w(TAG, "getMainDeviceType device.name=" + device.getName() + ",type=" + type);
        return type;
    ...
    public static boolean isPhone(BluetoothDevice device) {
        int mainDeviceType = getMainDeviceType(device);
        return mainDeviceType == BluetoothClass.Device.Major.PHONE;
    }
     ...
     * @return
     */
    public static boolean isHeadSet(BluetoothDevice device) {
        int mainDeviceType = getMainDeviceType(device);
        switch (mainDeviceType) {
            case BluetoothClass.Device.AUDIO_VIDEO_HEADPHONES://耳机
            case BluetoothClass.Device.AUDIO_VIDEO_WEARABLE_HEADSET://穿戴式耳机
            case BluetoothClass.Device.AUDIO_VIDEO_HANDSFREE://蓝牙耳机
            case BluetoothClass.Device.Major.AUDIO_VIDEO://音频设备
     ...
     * @return
     */
    public static boolean isHid(BluetoothDevice device) {
        int mainDeviceType = getMainDeviceType(device);
        switch (mainDeviceType) {
            case BluetoothClass.Device.AUDIO_VIDEO_VIDEO_GAMING_TOY://游戏
            case BluetoothClass.Device.Major.PERIPHERAL://外设
