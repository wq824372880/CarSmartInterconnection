package com.zeekrlife.connect.core.provider
...
import com.zeekrlife.connect.core.ui.activity.ProhibitWrapper
...
class HiCarAppListPropertiesProvider : ContentProvider() {
    ...
    companion object {
        ...
        const val AUTHORITY = "com.zeekrlife.connect.core.HiCarAppListPropertiesProvider"
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        "method:--->$method;arg:----->$arg,extras: ----->$extras".logE(TAG)
        var result = Bundle()
        try {
            when (method) {
                "notifyHiCarAppList" -> {
...
//                   result = setCallResult(cacheAppList)
                ...
                "startHiCarApp" -> {
                    extras?.let {
                        ...
                        "type:---->$type   ; isProhibitBylaw: ${ProhibitWrapper.instance.isProhibitBylaw}".logE(TAG)
                        if ("6" == type && !ProhibitWrapper.instance.isProhibitBylaw) {
                            MainScope().launch(Dispatchers.Main) {
                                ToastUtils.show(
                                    App.application, "为了驾驶安全,请在P档下使用",R.drawable.ic_toast_success,Toast.LENGTH_LONG

                                )
                            }
                        }else {
                            handleRequestStartApp(extras.getString("packageName") ?: "", type)
                        }
                    }
//                    arg?.let {
//                        handleRequestStartApp(it, "6")
//                    }
                }
            }
            return result
    ...
    private fun handleRequestStartApp(packageName: String,type:String,callBack: ((jsonString: String) -> Unit)? = null) {
        val bundle = Bundle().apply {
            putString("packageName",packageName)
            putString("type",type)
            putBoolean("RequestStartApp",true)
        }
        toStartActivity(SurfaceActivity::class.java,bundle)
    }
...
//
//        val uri = Uri.parse("content://com.zeekrlife.connect.core.HiCarAppListPropertiesProvider/query/all")
