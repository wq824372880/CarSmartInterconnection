...
object FileUtils {
    ...
    fun clearFolder(path: File): Int {
        ...
        if (fileList != null) {
            for (file in fileList) {
                ...
                if (file.delete()) {
                    Log.e("FileUtils", "clearFolder delete: ${file.delete()},deletedItems:${deletedItems+1}")
                    deletedItems++
                }
            }
        }
    ...
    fun delete(path: File?): Boolean {
        if (null == path) {
            Log.e("FileUtils", "delete: path is null.")
            return true
        }
        ...
        if (path.isDirectory) {
            ...
            if (null != files) {
                Log.e("FileUtils", "delete: path is not null.")
                for (file in files) {
                    if (!delete(file)) {
                        Log.e("FileUtils", "delete: delete fail${file.path}.")
                        return false
                    }
                }
            }
        }
        Log.e("FileUtils", "delete: !path.exists():${!path.exists()},path.delete:${path.delete()}")
